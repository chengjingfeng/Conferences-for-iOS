...
extension TalkModel {
    ...
    func matches(searchCriteria: String) -> Bool {
        ...
        if (components.count == 2 && SuggestionSourceEnum.isSource(text: components.first ?? "")) {
            ...
            return self.searchString.contains(searchCriteria.lowercased().replacingOccurrences(of: " ", with: ""))
        }
    }
}
...
extension TalkModel: ListRepresentable {
    var subtitle: String {
        return "\(speaker.firstname) \(speaker.lastname)"
    }

    var detail: String? {
        return tags.filter { !$0.contains("2019") && !$0.contains("2018") && !$0.contains("2017") && !$0.contains("2016")}.joined(separator: " • ")
    }
